...
def test_user_registration_duplicate_email(test_app, test_database, add_user):
    ...
    resp = client.post(
        "/auth/register",
        data=json.dumps(
            {"username": "miya", "email": "test@test.com", "password": "test"}
        ),
        content_type="application/json",
    )
    data = json.loads(resp.data.decode())
...
@pytest.mark.parametrize(
    ...
    [
        {},
        {"email": "me@yoyo.io", "password": "greaterthanten"},
        {"username": "miya", "password": "greaterthanten"},
        {"email": "me@yoyo.io", "username": "miya"},
    ],
)
