...
def test_single_user(test_app, test_database, add_user):
    user = add_user("jeffrey", "jeffrey@yoyo.io", "greaterthaneight")
    client = test_app.test_client()
...
def test_all_users(test_app, test_database, add_user):
    ...
    add_user("miya", "miya@mnazu.org", "greaterthaneight")
...
def test_remove_user(test_app, test_database, add_user):
    test_database.session.query(User).delete()
    user = add_user("user-to-be-removed", "remove-me@yoyo.io", "greaterthaneight")
    client = test_app.test_client()
...
def test_update_user(test_app, test_database, add_user):
    user = add_user("user-to-be-updated", "update-me@yoyo.io", "greaterthaneight")
    client = test_app.test_client()
    resp_one = client.put(
        f"/users/{user.id}",
...
def test_update_user_with_passord(test_app, test_database, add_user):
    ...
    user = add_user("user-to-be-updated", "update-me@yoyo.io", password_one)
    ...
    resp = client.put(
        f"/users/{user.id}",
        data=json.dumps(
            {"username": "me", "email": "foo@yoyo.io", "password": password_two}
        ),
        content_type="application/json",
    )
    assert resp.status_code == 200
